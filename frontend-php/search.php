...
$db = getDB();
if (!$db) {
    error_log("MongoDB connection failed in search.php");
...
<body>
    ...
    const debugPanelManager = {
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="editPropertyForm">
                                    ...
                                    <div class="mb-3">
                                        <label for="edit_coordinates" class="form-label">Coordinates</label>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <input type="text" class="form-control" id="edit_latitude" value="${propertyData.latitude || ''}" placeholder="Latitude">
                                            </div>
                                            <div class="col-md-6">
                                                <input type="text" class="form-control" id="edit_longitude" value="${propertyData.longitude || ''}" placeholder="Longitude">
                                            </div>
                                        </div>
                                        <div class="form-text">Current coordinates pre-loaded</div>
                                    </div>
                                </form>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-primary" onclick="debugPanelManager.savePropertyEdit()">Save Changes</button>
                                <button type="button" class="btn btn-info" onclick="debugPanelManager.regenerateThumbnail()">Regenerate Thumbnail</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        async savePropertyEdit() {
            ...
            if (latitude) formData += `&latitude=${encodeURIComponent(latitude)}`;
            if (longitude) formData += `&longitude=${encodeURIComponent(longitude)}`;
            ...
            try {
                ...
                if (data.status === 'success') {
                    alert('Property updated successfully! Page will refresh to show changes.');
        ...
        async regenerateThumbnail() {
            ...
            try {
                const response = await fetch(`/api/generate_thumbnails.php?action=regenerate&assessment_number=${assessmentNumber}`, {
                    method: 'POST'
                });
                
                const data = await response.json();
                if (data.status === 'success') {
                    alert('Thumbnail regeneration triggered! Page will refresh to show updated thumbnail.');
                    window.location.reload();
                } else {
                    alert('Thumbnail regeneration result: ' + (data.message || 'Check admin panel for details'));
                }
            } catch (error) {
